...
以下の文言は雛形。提示時は各選択肢に「この回答でループ設計がどう変わるか」を
1行添える(→の「使い道」をユーザー向けの言葉に直す)。
...
**Interfaces:**
- Produces: マーカー群 `{{GOAL_SUMMARY}}` `{{VERIFY_CMD}}` `{{EXPECTED}}` `{{MAX_ITER}}` `{{MAX_STALL}}` `{{PROGRESS_METRIC}}` `{{LOOP_TYPE}}` `{{REJECTED}}` `{{WHY}}` `{{TOKEN_BUDGET}}` `{{HOURS}}` `{{EVALUATOR}}` `{{ESCALATION}}` `{{FINDINGS_FILE}}` `{{KILL_SWITCH_PATH}}` `{{WATCH_CMD}}` `{{INTERVAL}}` `{{STEER_METHOD}}` `{{SCHEDULE_REMOVE_CMD}}` `{{REVIEW_CMD}}` `{{REVIEW_BUDGET}}` `{{EVAL_AXES}}` `{{MODEL_ASSIGNMENT}}`。SKILL.md Phase 4〜6 が使用
...
Expected: `23`(Interfaces欄のマーカー23種が両ファイル合計で過不足なく揃っている)
...
シナリオ4 — エピック級の分解提案:
- [ ] 「登録・ログイン・パスワードリセットを備えた認証機能一式」のようなエピック級
      ゴールで、Planner分解型アウターループの提案が出る(単一ループ続行の選択肢も
      提示されること)
